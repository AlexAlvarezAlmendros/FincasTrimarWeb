...
class JsonImportService {
  ...
  mergeForUpdate(existing, transformed) {
    return {
      ...
      // Si el inmueble pasa a ser no-vivienda (Terreno, Local…), limpiar el
      // TipoVivienda anterior en vez de conservarlo (?? mantendría "Piso")
      tipoVivienda: (transformed.tipoInmueble && transformed.tipoInmueble !== 'Vivienda')
        ? null
        : (transformed.tipoVivienda ?? existing.tipoVivienda),
   ...
   * Prioriza el título del anuncio (siempre empieza por el tipo y es más fiable
   * que el tipo_inmueble scrapeado, que a veces viene mal etiquetado) y usa
   * tipo_inmueble como respaldo. Gana la palabra clave que aparece ANTES en el
   * texto ("Piso con garaje" → Piso, "Garaje en venta" → Garaje). Matching con
   * límite de palabra para evitar falsos positivos ("localidad" no es "local").
   */
  inferirTiposDesdeInmueble(inmueble) {
    ...
    // Devuelve los tipos de la keyword que aparece antes en el texto, o null
    const inferirDesdeTexto = (texto) => {
      if (!texto) return null;
      const lower = texto.toLowerCase();
      const matchIndex = (keyword) => {
        const regex = new RegExp(`(?<![a-záéíóúüñ])${keyword}(?![a-záéíóúüñ])`, 'i');
      ...
      for (const [keyword, tipoInmueble] of Object.entries(noViviendaMap)) {
        const idx = matchIndex(keyword);
        if (idx !== -1 && (best === null || idx < best.idx)) {
          best = { idx, tipos: { tipoInmueble, tipoVivienda: null } };
        }
      }
      for (const [keyword, tipoVivienda] of Object.entries(viviendaMap)) {
        const idx = matchIndex(keyword);
        if (idx !== -1 && (best === null || idx < best.idx)) {
          best = { idx, tipos: { tipoInmueble: 'Vivienda', tipoVivienda } };
        }
      }
      return best ? best.tipos : null;
    };

    return inferirDesdeTexto(inmueble.titulo)
      ?? inferirDesdeTexto(inmueble.tipo_inmueble)
      ?? { tipoInmueble: 'Vivienda', tipoVivienda: 'Piso' };
  }
  ...
  transformJsonToVivienda(jsonVivienda) {
    ...
    // Misma inferencia de tipos que el formato agencia (admite no-vivienda)
    const { tipoInmueble, tipoVivienda } = this.inferirTiposDesdeInmueble(jsonVivienda);
